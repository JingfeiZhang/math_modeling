...
DATA = OUT
Q2_SCHEDULE = OUT / "q2_full_candidate_schedule.csv"
...
def prepare_modules():
    from code.q2 import scheduler_core as q2
    from code.q4 import storage_recourse as q4
    q2.DATA_DIR = DATA
    q2.STAGING = OUT
    q4.HORIZON, q4.WINDOW_START_H = HORIZON, H0
    return q2, q4
...
# BEGIN APPENDIX_Q4_FIX_AND_OPTIMIZE
...
# END APPENDIX_Q4_FIX_AND_OPTIMIZE


def main(argv: list[str] | None = None) -> int:
    global DATA, OUT, Q2_SCHEDULE, SEED
    parser = argparse.ArgumentParser(description="Q4 bounded joint task-storage optimization")
    parser.add_argument("--input-dir", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--q2-schedule", type=Path, required=True)
    parser.add_argument("--seed", type=int, default=SEED)
    args = parser.parse_args(argv)
    DATA, OUT, Q2_SCHEDULE, SEED = args.input_dir.resolve(), args.output_dir.resolve(), args.q2_schedule.resolve(), int(args.seed)
    ...
    inputs = [Q2_SCHEDULE, *(DATA / name for name in ("workload_trace.xlsx", "GPU_information.xlsx", "network_latency.xlsx", "region_time_data.xlsx", "power_mapping.xlsx", "storage_information.xlsx"))]
    dump(OUT / "run_manifest.json", {"schema_version": 1, "question_id": "Q4", "status": "PROBE_ONLY", "command": ["python", "code/q4/run_q4.py"], "seed": SEED, "inputs": [{"path": p.name, "sha256": sha256(p)} for p in inputs], "environment": {"python": sys.version, "platform": platform.platform(), "numpy": np.__version__, "pandas": pd.__version__}, "generated_at_utc": datetime.now(timezone.utc).isoformat(), "baseline_envelope": baseline_meta, "candidate_envelope": candidate_meta})
