#!/usr/bin/env python
"""Deterministic Q1 scheduling runner with an independent output directory."""
...
# BEGIN APPENDIX_Q1_SCHEDULING_CP_SAT
# BEGIN APPENDIX_Q1_CP_SAT_VARIABLES
...
# END APPENDIX_Q1_CP_SAT_VARIABLES
# BEGIN APPENDIX_Q1_CP_SAT_CAPACITY_SOLVE
...
# END APPENDIX_Q1_CP_SAT_CAPACITY_SOLVE
    rows: list[dict] = []
    for _, task in tasks.iterrows():
...
# END APPENDIX_Q1_SCHEDULING_CP_SAT
...
def main(argv: list[str] | None = None) -> int:
    global DATA, STAGING, SEED
    parser = argparse.ArgumentParser(description="Q1 minute-level CP-SAT scheduler")
    ...
    started = time.perf_counter()
    input_names = ["workload_trace.xlsx", "GPU_information.xlsx", "network_latency.xlsx", "region_time_data.xlsx", "power_mapping.xlsx"]
    input_hashes = [{"path": name, "sha256": sha256(DATA / name)} for name in input_names]
    ...
    # Baseline and optimized schedules share the same output schema.
    ...
    claim = {
        ...
        "claims": [
            ...
            },
            {
                ...
                "evidence": ["baseline_schedule.csv", "constraint_audit.json"],
            },
        ],
    }
    ...
    artifact_names = [
        "scheduling_metrics.json", "baseline_schedule.csv", "optimized_schedule.csv", "warmup_task_audit.csv",
        "resource_audit.csv", "carry_in.json", "constraint_audit.json", "fallback_test.json", "claim_proposal.json",
    ]
    artifacts = [{"path": name, "sha256": sha256(STAGING / name)} for name in artifact_names]
    run_manifest = {
        "schema_version": 1,
        "run_id": "q1-scheduling-submission",
        "problem_id": "C",
        "question_id": "Q1",
        "engine": "python",
        "command": ["python", "code/q1/run_q1.py"],
        "environment": {"python": platform.python_version(), "ortools": __import__("ortools").__version__, "platform": platform.platform()},
        "code": {"runner": "code/q1/schedule_core.py", "sha256": sha256(code_path)},
        "random_seed": SEED,
        "methods": [
    ...
    artifacts.append({"path": "run_manifest.json", "sha256": sha256(STAGING / "run_manifest.json")})
    ...
    artifacts.append({"path": "hash_manifest.json", "sha256": sha256(STAGING / "hash_manifest.json")})

    written = [item["path"] for item in artifacts] + ["handoff.json"]
    handoff = {
        "schema_version": 1,
        "run_id": "q1-scheduling-submission",
