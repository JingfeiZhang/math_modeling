...
RUN_FAMILY = "q3-storage-submission"
SEED = 20260808
TOL = 5e-5
...
# BEGIN APPENDIX_Q3_STORAGE_MILP
# BEGIN APPENDIX_Q3_STORAGE_OBJECTIVE_BOUNDS
def make_model(
    frame: pd.DataFrame,
    storage: pd.Series,
...
# END APPENDIX_Q3_STORAGE_OBJECTIVE_BOUNDS
# BEGIN APPENDIX_Q3_STORAGE_BALANCE
...
# END APPENDIX_Q3_STORAGE_BALANCE
# BEGIN APPENDIX_Q3_STORAGE_MUTEX_TERMINAL
    for t in range(horizon):
        matrix[row, slices["grid_load"].start + t] = 1.0
        matrix[row, slices["grid_charge"].start + t] = 1.0
        matrix[row, slices["sell"].start + t] = -1.0
...
# END APPENDIX_Q3_STORAGE_MUTEX_TERMINAL
# END APPENDIX_Q3_STORAGE_MILP
...
def main(argv: list[str] | None = None) -> int:
    global SEED
    parser = argparse.ArgumentParser(description="Q3 rolling binary storage MILP")
    ...
    input_names = ["region_time_data.xlsx", "storage_information.xlsx", "power_mapping.xlsx"]
    expected_inputs = [{"path": name, "kind": "file", "exists": True, "sha256": sha256_file(data_dir / name)} for name in input_names]

    region_data = pd.read_excel(data_dir / "region_time_data.xlsx", sheet_name="region_time_data", engine="openpyxl")
    storage_data = pd.read_excel(data_dir / "storage_information.xlsx", sheet_name="storage_information", engine="openpyxl")
    power_table = pd.read_excel(data_dir / "power_mapping.xlsx", sheet_name="任务功率映射", engine="openpyxl")
    power_rules = pd.read_excel(data_dir / "power_mapping.xlsx", sheet_name="计算口径", engine="openpyxl")
    ...
    summary = {
        "schema_version": 1, "run_family": RUN_FAMILY, "question": "Q3",
    ...
    manifest = {
        ...
        "command": "python code/q3/run_q3.py", "working_directory": "submission output directory",
        ...
        "code": {"path": "code/q3/storage_core.py", "sha256": code_hash},
    ...
    output_names = manifest["outputs"] + ["q3_run_manifest.json"]
    artifacts = [{"path": name, "sha256": sha256_file(out_dir / name)} for name in output_names if (out_dir / name).is_file()]
    handoff = {
        "schema_version": 1, "run_family": RUN_FAMILY,
        "status": summary["status"], "input_hashes": expected_inputs,
        "written_paths": [a["path"] for a in artifacts] + ["handoff.json"],
