...
TASK_PACKAGE_PATH = ROOT / "sprints" / SPRINT_ID / "tasks" / "solver-q1.json"
PINNED_RUNNER = (
    ROOT
    / "sprints"
...
def main() -> int:
    ...
    if sha256(PINNED_RUNNER) != next(
        item["sha256"]
        for item in task_package["input_hashes"]
        if item["path"].endswith("merged/solver-q2/run_q2_full_horizon.py")
    ):
        raise RuntimeError("pinned Q2 runner hash does not match task package")
