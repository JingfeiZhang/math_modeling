...
def read_workload(project_root: Path) -> tuple[pd.DataFrame, pd.DataFrame, np.ndarray]:
    path = project_root / "problems/C/data/workload_trace.xlsx"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Q1 hierarchical GPU-workload forecast worker")
    ...
    if task.get("sprint_id") != SPRINT_ID:
        raise ValueError(f"runner is pinned to {SPRINT_ID}, got {task.get('sprint_id')!r}")
    if task.get("task_id") != "forecast-q1":
        raise ValueError(f"runner requires forecast-q1 task, got {task.get('task_id')!r}")
    output = (project_root / task["write_directory"]).resolve()
    output.relative_to(project_root)
    if output.name != "forecast-q1" or output.parent.name != "staging":
        raise ValueError("runner refuses a write directory outside forecast-q1 staging")
    output.mkdir(parents=True, exist_ok=True)
    started = datetime.now(UTC)
    tick = time.perf_counter()
    np.random.seed(SEED)

    tasks, panel, actual = read_workload(project_root)
    ...
    run_manifest = {
        "schema_version": 1, "run_id": f"q1-forecast-{SPRINT_ID}", "sprint_id": SPRINT_ID, "problem_id": "C", "question_id": "Q1",
        "engine": "python",
        "command": [sys.executable, "-s", project_relative(project_root, runner_path), "--project-root", str(project_root), "--task", project_relative(project_root, task_path)],
        "environment": {
