...
def find_project_root(path: Path) -> Path:
    for parent in (path, *path.parents):
        if (parent / "contest.yaml").is_file() and (parent / "problems/C/data").is_dir():
            return parent
    raise RuntimeError("cannot locate Huashu Cup project root")


SCRIPT = Path(__file__).resolve()
ROOT = find_project_root(SCRIPT.parent)
BASE_RUNNER = ROOT / "sprints/sprint-20260808T023236665505Z/merged/solver-q3/run_solver_q3.py"
LEGACY_AUDIT = ROOT / "sprints/sprint-20260808T023236665505Z/merged/solver-q3/q3_constraint_audit.json"
LEGACY_SUMMARY = ROOT / "experiments/C/Q3/q3-rolling-compat-20260808/q3_derived_summary.json"
TASK_PACKAGE = ROOT / "sprints/sprint-20260808T023236665505Z/tasks/solver-q3.json"
...
def load_module(path: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("q3_frozen_runner", path)
...
def verify_pinned_inputs(q3: Any) -> list[dict[str, Any]]:
    task = load_json(TASK_PACKAGE)
    expected = list(task["input_hashes"])
    records: list[dict[str, Any]] = []
    for item in expected:
        current = q3.fingerprint(ROOT, item["path"])
        match = current == item
        is_current_manifest = item["path"] == "problems/C/questions/Q3/question.yaml"
        if not match and not is_current_manifest:
            raise RuntimeError(f"Q3 task input changed: {item['path']}")
        records.append(
            {
                "path": item["path"],
                "expected_at_original_run": item,
                "current": current,
                "match": match,
                "accepted_current_manifest": bool(is_current_manifest and not match),
            }
        )
    return records
...
def generate(output_dir: Path) -> int:
    ...
    q3 = load_module(BASE_RUNNER)
    q3.TIME_LIMIT_S = TIME_LIMIT_S
    pinned_inputs = verify_pinned_inputs(q3)
    region_data, storage_data = load_inputs()
    legacy_doc = load_json(LEGACY_AUDIT)
    ...
    summary = {
        ...
        "sources": {
            "base_runner": {"path": relative(BASE_RUNNER), "sha256": sha256_file(BASE_RUNNER)},
            "legacy_audit": {"path": relative(LEGACY_AUDIT), "sha256": sha256_file(LEGACY_AUDIT)},
            "legacy_summary": {"path": relative(LEGACY_SUMMARY), "sha256": sha256_file(LEGACY_SUMMARY)},
            "task_package": {"path": relative(TASK_PACKAGE), "sha256": sha256_file(TASK_PACKAGE)},
    ...
    manifest = {
        ...
        "command": [sys.executable, relative(SCRIPT), *sys.argv[1:]],
        ...
        "code": {"runner": relative(SCRIPT), "sha256": sha256_file(SCRIPT)},
        "methods": [
            {
                "role": "main",
                "name": "regional full-horizon binary MILP audit probe",
                "implementation": relative(SCRIPT),
            ...
            },
            {
                ...
                "implementation": relative(BASE_RUNNER),
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Project-relative or absolute output directory; defaults to the runner directory.",
    )
    parser.add_argument(
        "--verify-replay-dir",
        default=None,
