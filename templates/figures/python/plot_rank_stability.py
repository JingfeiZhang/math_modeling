...
def main() -> int:
    ...
    ax.imshow(matrix, aspect="auto", interpolation="nearest", cmap="Blues_r", vmin=1, vmax=max(2.0, float(np.max(matrix))))
    ...
    for row in range(matrix.shape[0]):
        for col in range(matrix.shape[1]):
            ...
            ax.text(col, row, f"{int(value)}", ha="center", va="center", fontsize=8, color=COLORS["ink"])
